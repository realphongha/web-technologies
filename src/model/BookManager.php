...
class BookManager{
    ...
    public function countBooks($keyword){
        $query = "SELECT count(book_id) FROM book"
                . " WHERE status = " . strval(BOOK_ACTIVE)
                . (is_null($keyword)?"":" AND (title LIKE CONCAT('%',?,'%') OR "
                        . "author LIKE CONCAT('%',?,'%') OR "
                        . "category LIKE CONCAT('%',?,'%') OR "
        ...
        if (!is_null($keyword)){
            $conn->bind_param("ssss", $keyword, $keyword, $keyword, $keyword);
        }
        $conn->execute();
        $result = $conn->get_result();
        if ($result) {
            $count = $result->fetch_assoc()["count(book_id)"];
            return $count;
        } else {
            return null;
        }
