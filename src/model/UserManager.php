...
//        echo $query;
        ...
        if (!is_null($keyword)){
            $conn->bind_param("sss", $keyword, $keyword, $keyword);
        }
        $conn->execute();
        $result = $conn->get_result();
        if ($result) {
            $count = $result->fetch_assoc()["count(user_id)"];
            return $count;
        } else {
            return null;
        }
...
//            echo $this->db->errno;
    ...
    public function editUser($userId, $email, $hashedPassword, $name, $icNumber,
        ...
        $query = "UPDATE user SET "
                . "email = ?,"
                . (is_null($hashedPassword)?"":"password = ?,")
                . "name = ?," 
                . "ic_number = ?,"
                . "phone = ?,"
                . "date_of_birth = ?,"
        ...
        if (is_null($hashedPassword)){
            $conn->bind_param("ssssssi", $email, $name, $icNumber,
    ...
    public function changePassword($email, $hashedPassword){
        $query = "UPDATE user SET password = ? WHERE email = ?";
        if (!$conn = $this->db->prepare($query)){
            return false;
        }
        $conn->bind_param("ss", $hashedPassword, $email);
        return $conn->execute();
    }
