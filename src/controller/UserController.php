...
class UserController extends BaseController{
    ...
    public function list($request, $queries){
        ...
        $pagination = new Pagination($queries["page"], sizeof($users),
        ...
        $View->renderView($message, array($users, $pagination));
    }
...
//            echo "Khong tim thay user nao";
    ...
    public function add($request, $queries){
        ...
        $fileName = date("m-d-y") . date("h-i-sa") 
                ...
                . ".$removeExtension[1]";
        $staticFilePath = STATIC_IMG_DIRECTORY_USER . $fileName;
        $dbFilePath = IMG_DIRECTORY_USER . $fileName;
    ...
    public function update($request, $queries){
        ...
        if($this->userManager->editUser(
                $queries["id"], 
                $request["email"], 
                is_null($request["password"])?null:encryptPassword($request["password"]), 
                $request["name"], 
                $request["icNumber"], 
                $request["phone"], 
                $request["dateOfBirth"],
